...
/**
 * Validator for jvm.options. Each line must be blank, a comment (#), or an entry. Entries must not start or end with whitespace, and
        ...
        while (true) {
            if ('#' == nextChar) { // comment
                ...
                if (t.length() == 0)
                    ...
                    if (!t.startsWith("-")) {
                        int x = charCount - s.length();
                        result.add(ConfigurationValidator.createMessage(file, Messages.invalidJVMOption, line, x, x + 1));
                    }
                }
