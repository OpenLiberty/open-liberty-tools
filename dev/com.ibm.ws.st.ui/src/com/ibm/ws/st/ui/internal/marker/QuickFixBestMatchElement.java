...
public class QuickFixBestMatchElement extends AbstractMarkerResolution {
    ...
    public QuickFixBestMatchElement(String bestMatch) {
        this.bestMatch = bestMatch;
    }

    @Override
    public String getLabel() {
        return NLS.bind(Messages.unrecognizedElementQuickFix, new String[] { bestMatch });
    }
    ...
    public void run(IMarker marker) {
        ...
        if (configFile.changeElementName(xpath, bestMatch)) {
            try {
                configFile.save(null);
            } catch (IOException e) {
                if (Trace.ENABLED)
                    Trace.trace(Trace.ERROR, "Quick fix for element not found failed. Error trying to save configuration: " + configFile.getURI(), e);
                showErrorMessage();
            }
        } else {
            if (Trace.ENABLED) {
                Trace.trace(Trace.ERROR,
                            "Quick fix for element not found failed. Error trying to change element name '" + xpath + "' in configuration: " + configFile.getURI(), null);
            }
            showErrorMessage();
        }
    }
